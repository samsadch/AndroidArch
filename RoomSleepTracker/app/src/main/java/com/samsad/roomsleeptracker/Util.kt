...
fun formatNights(nights: List<SleepNight>, resources: Resources): Spanned {
    ...
    sb.apply {
        ...
        nights.forEach {
            ...
            if (it.endTimeInMilli != it.startTimeInMilli) {
                append(resources.getString(R.string.end_time))
                append("\t${convertLongToDateString(it.endTimeInMilli)}<br>")
                append(resources.getString(R.string.quality))
                append("\t${convertNumericQualityToString(it.sleepQuality, resources)}<br>")
                append(resources.getString(R.string.hours_slept))
                // Hours
                append("\t ${it.endTimeInMilli.minus(it.startTimeInMilli) / 1000 / 60 / 60}:")
                // Minutes
                append("${it.endTimeInMilli.minus(it.startTimeInMilli) / 1000 / 60}:")
                // Seconds
                append("${it.endTimeInMilli.minus(it.startTimeInMilli) / 1000}<br><br>")
            }
        }
    }
